...
dashboard/     → DataVerse_Omnichannel.pbix — modelo Power BI (Fact_Orders
                 importado desde silver + medidas DAX que replican src/kpis.py)
...
4. **Typos de categoría** — se detectaron errores de tipeo en `app`, `retail`
   ...
   formas correctas), corregidos con diccionario de normalización. Los typos
   de retail y franchise se detectaron durante la validación de la capa
   silver, revisando valores únicos de categoría post-transformación, no en
   el EDA inicial.
...
6. **adiClub** — normalizado a booleano. En `app`, el 100% de los registros
   es `True`: la app requiere cuenta registrada para comprar, lo que asocia
   automáticamente al usuario a adiClub. Asunción de negocio pendiente de
   validar con el equipo del canal.
...
Las tasas 2 y 3 muestran muy poca varianza entre país, canal, categoría y
mes, consistente con cifras sintéticas. El patrón de negocio real está en
la dimensión adiClub (sección 7), no en estas tasas.

### Vista ejecutiva — cortes principales

![Revenue total por país](images/revenue_by_country.png)
![Revenue total por canal](images/revenue_by_channel.png)
![Tendencia de revenue por mes](images/revenue_trend_by_month.png)
![Tasa de devolución por canal](images/return_rate_by_channel.png)

Generadas desde `data/gold/`. La versión interactiva, con slicers de país,
canal, categoría, mes y adiClub, está en `dashboard/DataVerse_Omnichannel.pbix`.
...
más por compra", descartada: el ticket promedio por orden es prácticamente
idéntico entre grupos (€347.97 miembros vs €359.45 no-miembros). El driver
real es **frecuencia de compra**: los miembros generaron 553,504 órdenes vs
325,001 de no-miembros, 1.70x en el agregado y 1.22x-1.24x en retail,
ecommerce y franchise por separado. El agregado sube porque incluye `app`,
canal 100% adiClub sin contraparte no-miembro.

**Recomendación:** dado que el ticket promedio es equivalente entre grupos,
la oportunidad no está en upsell sino en crecer la base de miembros adiClub
con campañas de inscripción en checkout y punto de venta. Cada conversión
representa un incremento predecible de frecuencia de compra, no un cambio
de comportamiento de gasto que haya que inducir.
...
![Comparativo adiClub: órdenes y ticket promedio](images/adiclub_comparison.png)

En `dashboard/DataVerse_Omnichannel.pbix`, el 10% es el parámetro
`Migracion %`, conectado a la medida `Revenue Incremental Potencial` para
mover el escenario en vivo.
...
- **Adopción** — las tablas gold en Parquet se conectan directo a Power BI,
  ya estándar en el equipo, evitando imponer una herramienta nueva a
...
Para el dashboard: abrir `dashboard/DataVerse_Omnichannel.pbix` en Power BI
Desktop y refrescar (`Fact_Orders` lee directo `data/silver/fact_orders.parquet`).
